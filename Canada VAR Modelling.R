...
for (i in c(1:6)) {
  # Data Set
  dataset = read_dta("CANVAR.dta")
  dataset = as.data.frame(dataset)
  dataset$investment = dataset$investment * 100
  
  # Selecting Relevant Variables
  data_investment = dataset %>% dplyr::select(c(canrsmv, usrsmv, vix, canepu, usepu, srv, tsx_dlog, tobinsq, salesgrowth, cashflow, canbankrate, investment, fqtr, canelectionyear, date, leverage))
  ...
  variablename_table_df1 = reshape2::melt(variablename_table_df, id.vars = "row", measure.vars = c(uncertainty,"salesgrowth","cashflow","investment","canbankrate","tsx_dlog","tobinsq"))
  ...
  condition = rep(c("Uncertainty","Investment Rate","Monetary Policy Rate","Stock Market"), 20)
...
for (i in c(1:6)) {
  ...
  data_productivity = dataset %>% dplyr::select(c(canrsmv, usrsmv, vix, canepu, usepu, srv, tsx_dlog, tobinsq, salesgrowth, cashflow, canbankrate, productivitygrowth, fqtr, canelectionyear, date, leverage))
  ...
  variablename_table_df$row = rownames(variablename_table_df)
  variablename_table_df1 = reshape2::melt(variablename_table_df, id.vars = "row", measure.vars = c(uncertainty,"salesgrowth","cashflow","productivitygrowth","canbankrate","tsx_dlog","tobinsq"))
  colnames(variablename_table_df1) = c("period","variable","value")
...
for (i in c(1:6)) {
  ...
  # Keeping Relevant Variables
  data_bankruptcy = dataset %>% dplyr::select(c(canrsmv, usrsmv, vix, canepu, usepu, srv, tsx_dlog, tobinsq, salesgrowth, cashflow, canbankrate, bankruptcy, fqtr, canelectionyear, date, leverage))
  ...
  variablename_table_df$row = rownames(variablename_table_df)
  variablename_table_df1 = reshape2::melt(variablename_table_df, id.vars = "row", measure.vars = c(uncertainty,"salesgrowth","cashflow","bankruptcy","canbankrate","tsx_dlog","tobinsq"))
  colnames(variablename_table_df1) = c("period","variable","value")
